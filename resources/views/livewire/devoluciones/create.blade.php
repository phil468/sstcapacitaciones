            ...
            <div @if (!$createMode || $pdfMode) style="display:none" @endif>
                    ...
                    <div>
                            ...
                            @if (!$viewMode)
                                <div class="form-group col-sm-4">
                                    ...
                                    <div class="input-group">
                                      <input type="text" inputmode="numeric" name="search_personal_dni" wire:model.defer="search_personal_dni"
                                          wire:keydown.enter='buscar_dni' id="search_personal_dni" class="form-control form-control-lg"
                                          wire:keydown.tab="buscar_dni"
                                          wire:keydown.arrow-right="buscar_dni"
                                          placeholder="DNI" aria-describedby="button-addon2">
                                      <a wire:click="buscar_dni()" type="button" class="btn btn-lg btn-primary" id="button-addon2"><i class="fas fa-search"></i></a>
                                    </div>
                                </div>
                                ...
                                <div class="form-group col-sm-4">
                                    ...
                                    <div class="input-group">
                                      <a wire:click="listarSelects()" type="button" class="btn btn-lg btn-primary" id="button-addon2">Recargar <i class="fas fa-sync-alt"></i></a>
                                    </div>
                                </div>
                            @endif
                        </div>
                        ...
                        <div class="row">
                            <h4>
                                <div class="mt-3 h4 col-xs-12">Fecha de Devolución</div>
                            </h4>
                        </div>
                        <div class="row">
                            <div class="form-group col-sm-4">
                        ...
                        @if (!$viewMode)
                            <div class="mt-3 row">
                                <h4>
                                    <div class="h4 col-xs-12">Activos Asignados</div>
                                </h4>
                            </div>
                            <div class="row">
                                
                                <div class="form-group col-sm-4">
                                    <label for="activo_id">Buscar activo asignado por S/N o IMEI *</label>
                                    <div class="input-group">
                                    ...
                                    @error('activo_id')
                                        <span class="error text-danger">{{ $message }}</span>
                                    @enderror
                                    @error('activos_asignados')
                                        <span class="error text-danger">{{ $message }}</span>
                                    @enderror
                                </div>
                                
                                <div class="form-group col-sm-4">
                                    <label for="activo_precargado_id">Seleccionar activo de la lista por S/N o IMEI *</label>
                                    <div class="input-group">
                            ...
                            <table class="table table-striped table-hover table-sm">
                                <thead style="{z-index: 1 ;}">
                                    <tr>
                                        ...
                                        @endcan
                                        <th>Activo</th>
                                        <th class="table-warning">Condicion Devolución</th>
                                        <th>Accesorios Entregados</th>
                                        <th class="table-warning">Accesorios Devueltos</th>
                                        <th>Vigencia</th>
                                        <th>Fecha Vigencia</th>
                                        <th>Obs. Activo</th>
                                        <th>Obs. Asignación</th>
                                        <th class="table-warning">Obs. Devolución</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @if ($activos_asignados)
                                        @foreach ($activos_asignados as $index => $row)
                                            {{-- @if ($row['eliminado'] == 0) --}}
                                                <tr>
                                                        @if (!$viewMode)
                                                            <td width="45">
                                                                ...
                                                                @if ($row['seleccionado'] == 0)
                                                                    <a wire:click="seleccionar_activo({{$index}})" 
                                                                        type="button" class="btn btn-lg btn-default" title="Seleccionar activo"
                                                                        >
                                                                        <i class="far fa-square fa-lg"></i>
                                                                    </a>
                                                                @else
                                                                    <a wire:click="deseleccionar_activo({{$index}})" 
                                                                        type="button" class="btn btn-lg btn-primary" title="Deseleccionar activo"
                                                                        >
                                                                    ...
                                                                    <a data-toggle="modal"
                                                                        ...
                                                                        class="btn btn-lg btn-warning" title="Editar entrega"
                                                    ...
                                                    <td>
                                                        {{ $row['asignacion_has_activo']['performance']['name'] ?? '' }}
                                                        @error('activos_asignados.'.$index.'.asignacion_has_activo.performance.name')
                                                            <span class="error text-danger">{{ $message }}</span>
                                                        @enderror
                                                    </td>
                                                    <td>
                                                        @if (!empty($row['asignacion_has_activo']['accesorios']))
                                                            {{ implode(', ', array_column($row['asignacion_has_activo']['accesorios'],'name')) }}
                                                        @endif
                                                    </td>
                                                    <td>
                                                        @if (!empty($row['asignacion_has_activo']['accesorios_devueltos_names']))
                                                            {{ implode(', ', $row['asignacion_has_activo']['accesorios_devueltos_names']) }}
                                                        @endif
                                                    </td>
                                                    <td>
                                                        {{ $row['asignacion_has_activo']['vigencia']['name'] ?? '' }}
                                                        {{-- @error('activos_asignados.'.$index.'.asignacion_has_activo.vigencia.name')
                                                            <span class="error text-danger">{{ $message }}</span>
                                                        @enderror --}}
                                                    </td>
                                                    <td>
                                                        {{ $row['asignacion_has_activo']['fecha_vigencia'] ?? '' }}
                                                    </td>
                                                    <td>
                                                        {{ $row['observations'] ?? '' }}
                                                    </td>
                                                    <td>
                                                        {{ $row['asignacion_has_activo']['observaciones'] ?? '' }}
                                                    </td>
                                                    <td>
                                                        {{ $row['observaciones_devolucion'] ?? '' }}
                                                    </td>
                                                </tr>
                                            {{-- @endif --}}
                                        @endforeach
                ...
                <table border="0" cellpadding="0" cellspacing="0" id="sheet0" class="sheet0 gridlines">
                        ...
                        <tbody>
                            <tr class="row0">
                                ...
                                <td class="column1 style2 s" colspan="6">ACTA DE DEVOLUCION DE ACTIVOS DE TI
                                </td>
                            </tr>
                            <tr class="row1">
                            ...
                            <tr class="row14">
                                <td class="column0 style18 s">RESPONSABLE TI</td>
                                <td class="column1 style19 s" colspan="6">
                            ...
                            <tr class="row20">
                                ...
                                <td class="column5 style33 s">OBSERVACIONES</td>
                            </tr>
                            @foreach ($activos_asignados as $index => $activo)
                                @if ($activo['seleccionado'] == 1)
                                    <tr class="row21">
                                        <td class="column0 style34 null" colspan="2">{!! $activo['descripcion'] !!} </td>
                                        <td class="column1 style34 null" colspan="2">
                                            @if (!empty($activo['asignacion_has_activo']['accesorios_devueltos_names']))
                                                {{ implode(', ',($activo['asignacion_has_activo']['accesorios_devueltos_names'])) }}                                                 
                                            @endif                                  
                                        </td>
                                        <td class="column2 style34 null" colspan="2">
                                            {{ $activo['asignacion_has_activo']['performance']['name'] }} 
                                        </td>
                                        <td class="column5 style35 null">{{ $activo['observaciones_devolucion'] }}</td>
                                    </tr>
                                @endif
                            ...
                            <tr class="row29" rowspan="4">
                                <td class="column0 style39 null" colspan="3" rowspan="4">                                  
                                    ...
                                    <a class="btn btn-lg btn-default" data-toggle="modal" data-target="#firmaModal"
