...
class TipoDePersonalController extends Controller
{
    ...
    public function obtenerResponse(string $empresa,string $val) {
        ...
        ])->get(config('app.url_api').'api/manager/tipodepersonal/'.$empresa.'/'.$val);
    ...
    public function upsert(string $empresa,string $val) 
    {
        ...
        if ($response2->successful()) {
            ...
            if(!$data) {
                ...
                foreach ($data as $row) 
                {
                    ...
                    if(!empty(trim($row['DESCRIPCION']))){
                        $planilla = TipoDePersonal::updateOrCreate(
                            ['idtipopersonal_nisira' => trim($row['IDTIPOPERSONAL']), 'empresa_id' => trim($row['EMPRESA'])],
                ...
                if($val != 0) {
                    if ($planilla) {
                        return $message = 'TIPO DE PERSONAL '. $val.' importados desde NISIRA.';
                    } else {
            ...
            if($val == 0) {
                ...
                return $message = 'Consulta TIPO DE PERSONAL '. $val.' devolvió error. Error: '.$statusCode;
            }
    ...
    public function actualizarNombreParaTodos() {
        ...
        $modelo = TipoDePersonal::all(); // Reemplaza 'TuModelo' por el nombre de tu modelo
        ...
        foreach ($modelo as $registro) {
            if($registro->name == NULL) {
                ...
                foreach ($jsonData as $jsonRegistro) {
                    if ($jsonRegistro['EMPRESA'] == $registro->empresa_id && $jsonRegistro['IDTIPOPERSONAL'] == $registro->idtipopersonal_nisira) {
                ...
                if ($registro->isDirty('name')) {
                    $registro->save();
                    $message = $message.'Se ingresó el tipo de personal: '.$registro->name.'<br>';
                }
            }
